...
def get_divisions(max_dividend: int=100, max_divisor: int=100) -> list[list]:
    ...
    Returns
    -------
        A two dimentional list with the operation on a string format and the result on float format
    """
    results = []

    for i in range(max_dividend + 1):
        for y in range(max_divisor + 1):
            if y > 0: # division by zero
                if i % y == 0: # get only integer divisors
                    # print(f"{i} ÷ {y} = {i / y}")
                    results.append([
                        f"{i} ÷ {y}", i // y
                    ])
